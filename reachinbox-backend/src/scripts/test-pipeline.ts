...
async function runEndToEndTest() {
  ...
  console.log(`\n1 Inserting email into PostgreSQL: ${emailId}`);
  await pool.query(
    `
  ...
  console.log(`\n2 Adding delayed BullMQ job for email: ${emailId}`);
  ...
  console.log('\n3 Waiting for BullMQ worker to process the job...');
  ...
  while (Date.now() - startTime < 25000) {
    ...
    console.log(` [${Math.round((Date.now() - startTime) / 1000)}s] Status in PostgreSQL: ${row.status}`);
