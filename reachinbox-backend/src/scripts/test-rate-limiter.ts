...
async function runRateLimiterTest() {
  ...
  console.log('1 Setting up test campaign:');
  ...
  for (let i = 0; i < recipients.length; i++) {
    ...
    const scheduledTime = new Date(now + (i * 500));
  ...
  console.log('\n2 Starting BullMQ Worker...');
  const worker = startEmailWorker();
  await worker.waitUntilReady();
  console.log(' BullMQ Worker started and listening.\n');

  console.log('3 Monitoring dispatch and rate limit enforcement...');
  ...
  while (Date.now() - monitoringStart < 15000) {
    ...
    console.log(
      ` [${Math.round((Date.now() - monitoringStart) / 1000)}s] Statuses: ` +
  ...
  console.log('\n4 Inspecting Redis Key details:');
  ...
  console.log('\n5 Verifying Rolling Window Expiration & Slot Reuse:');
  ...
  if (zsetEntries.length >= 2) {
    ...
    const expiredTimestamp = Date.now() - (60 * 60 * 1000 + 5000);
  ...
  while (Date.now() - slotWaitStart < 15000) {
    ...
    console.log(` [${Math.round((Date.now() - slotWaitStart) / 1000)}s] Email 3 status: ${email3FinalStatus}`);
    if (email3FinalStatus === 'sent') {
      break;
    }
  }

  console.log('\n6 Final Database Status & Duplicate Verification:');
